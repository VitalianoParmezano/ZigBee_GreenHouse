# 🌿 Комп'ютерна система керування освітленням теплиці (ZigBee GreenHouse)
...
---
## Задачі/проблеми які не завершились
...
3. Не перевірена синхронізація на усіх пристроях хто підключений до HA
4. Не до кінця чистий код, треба відрефакторити. Прибрати debug логи.
5. Надсилання boot_status кожні кілька хвилин поки не буде відповіді, якщо ж її немає тоді увійти в режим device reset 
6. Обробка відсутності з'єднання p.s. навіть якщо координатор від'єднається від живлення пристрій всерівно знайде мережу.

## Запуск (Docker)

Для запуску треба поставити контейнери `Home assistant server`, `Zigbee2MQTT server` та `Mosquitto` безпосередньо сам MQTT сервер.

> **Важливо:** Для коректної роботи кастомної логіки теплиці необхідно підключити зовнішній конвертер та розширення.

Конфіг для Zigbee2MQTT, ось приклад. Важливо щоб serial port співпадав з ІП самого пристрою, або ж local DNS. Також тут реєстрація external_converter та extension. Вони потім додаються усередину Volume за шляхами  /external_converters/greenhouse_controller.js та /external_extensions/greenhouse_grouper.js
```yaml
homeassistant:
  enabled: true
frontend:
...
extension:
  - greenhouse_grouper.js
device_options:
  retain: true
```

На стороні HomeAssistant має бути встановлене розширення `Zigbee2MQTT`. І також правильно імпортована картка, за це треба буде поговорити з AI, і потім в самому дашборді її викликати.
```yaml
type: custom:greenhouse-zone-card
```